...
export default function App(props) {
  ...
  return (
    <div className="chose-shelon-chosen">
      ...
      {props.chosen &&
        props.chosen.ans.map((q) => {
          indexQ++;
          return (
            <Quatsion
              quatsion={shelon ? shelon.list[indexQ] : null}
              fileId={props.chosen._id}
              indexQ={indexQ}
              q={q}
            />
          );
  ...
  if (edit) {
    return (
      <div
        ...
        className="chose-shelon-q-open"
      >
        <h1>{props.quatsion.quatsion}</h1>
        <input
          onChange={(e) => setEditDetails(e.target.value)}
          value={props.q && editDetails}
          className="chose-shelon-container2"
        />

        <input
          onChange={(e) => setEditFound(e.target.value)}
          value={props.q && editFound}
          className="chose-shelon-container2"
        />
