...
export default function Quatsion(props) {
  ...
  const [details, setDetails] = useState(
    props.ans[props.quatsion?.index]?.details
  );
  ...
  if (open) {
    ...
    return (
      <OnTop
        element={
          <QOnTop
            q={props.quatsion.quatsion}
            setOpen={setOpen}
            found={props.ans[props.quatsion.index]?.found}
            details={props.ans[props.quatsion.index]?.details}
    ...
    return (
      <div onClick={() => setOpen(true)} className="q-list1-open-q1">
        <span className="q-list1-open-q-text11">
          {getPartOfText(props.quatsion.quatsion)}
        </span>
      </div>
    );
  }
