  ...
  function selectedFromSerch(selected) {
    let el = (
      <Quatsion
        ...
        setAns={setAns}
        key={selected.id}
        quatsion={shelon.list[selected.id]}
      />
    );
  ...
  return (
    <div className="q-list1-open-container">
      <div className="q-list1-open-shelon">
        ...
        <Serch
          onSelect={(selected) => selectedFromSerch(selected)}
          items={getItmesFromShelon(shelon.list)}
        />
        {selected}
...
function ChoseShelon(props) {
  ...
  let list = props.shelons.map((shelon) => (
    <div
      onClick={() => props.setShelon(shelon)}
      className={chackIfChosen(shelon)}
    >
  ...
  return (
    <div className="q-list1-open-chose-shelon">
      <div className="q-list1-open-list-shelon">{list}</div>
    </div>
  );
}
