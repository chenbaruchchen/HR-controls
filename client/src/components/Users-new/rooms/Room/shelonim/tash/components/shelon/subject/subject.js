...
const Subject = (props) => {
  ...
  function getValByIndex(chosenSubjectIndex,index) {
    let subjectIndex =chosenSubjectIndex;
...
function Quatsion(props) {
  // const [value, setValue] = useState(props.ans[props.subjectIndex][props.index]?.value);
  const [value, setValue] = useState(props.ans?.[props.subjectIndex]?.[props.index]?.value);
  
  let valNew=props.getValByIndex(props.subjectIndex,props.index)
  console.log(valNew)
  ///update q to its paremater

  let input;
  let val=props.ans?.[props.subjectIndex]?.[props.index]?.value
