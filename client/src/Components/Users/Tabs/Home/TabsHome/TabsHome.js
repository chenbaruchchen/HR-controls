import React from "react";

import "./tabs-home.css";

const TabsHome = () => {
  return (
    <header className="tabs-home-tabs">
      <svg viewBox="0 0 1024 1024" className="tabs-home-icon">
        <path d="M512 854q140 0 241-101t101-241-101-241-241-101-241 101-101 241 101 241 241 101zM512 86q176 0 301 125t125 301-125 301-301 125-301-125-125-301 125-301 301-125zM426 704v-384l256 192z"></path>
      </svg>
      <svg viewBox="0 0 1024 1024" className="tabs-home-icon2">
        <path d="M512 854q140 0 241-101t101-241-101-241-241-101-241 101-101 241 101 241 241 101zM512 86q176 0 301 125t125 301-125 301-301 125-301-125-125-301 125-301 301-125zM426 704v-384l256 192z"></path>
      </svg>
      <svg viewBox="0 0 1024 1024" className="tabs-home-icon4">
        <path d="M512 854q140 0 241-101t101-241-101-241-241-101-241 101-101 241 101 241 241 101zM512 86q176 0 301 125t125 301-125 301-301 125-301-125-125-301 125-301 301-125zM426 704v-384l256 192z"></path>
      </svg>
    </header>
  );
};

export default TabsHome;
