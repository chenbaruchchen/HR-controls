...
{      sortedChat!==undefined &&  chat.map((m,i)=>{
  
  

    return(m.user.email===user.email)?<MyMsg massege={m}/>:<OtherMsg massege={m}/>
})
}   
 <MyMsg></MyMsg>
    ...
    <br/>  <br/><br/>  <br/>
    <SendMassege send={()=> sendMessage(room,message,user)} setMessage={setMessage} message={message}/>
  </div>
